...
- An [**_engine_**]({{site.url}}{{site.baseurl}}/mappings/supported-field-types/knn-methods-engines/) is the underlying library that implements vector search methods. It determines how vectors are indexed, stored, and retrieved during similarity search operations. OpenSearch supports multiple engines through the built-in k-NN plugin. The [`opensearch-jvector`]({{site.url}}{{site.baseurl}}/install-and-configure/additional-plugins/opensearch-jvector/) plugin provides an additional `jvector` engine, which implements DiskANN-style approximate nearest neighbor search with native Product Quantization (PQ) support, thread-safe concurrent ingestion, and incremental index updates.
